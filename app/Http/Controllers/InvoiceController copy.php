...
//---------Invoice new functions starts from here---------------------//
    ...
    public function index(Request $request) {
        ...
        $ptype = 'all';
        if (!empty($request->input('type'))) {
            $invoicedata = SumbInvoiceDetails::where('user_id', $userinfo[0])->paginate($itemsperpage)->toArray();
    ...
    public function invoiceForm($request){
        ...
        // echo "<pre>"; var_dump($pagedata['invoice_details']); echo "</pre>";die();
        return $pagedata;
        // return view('invoice.invoicecreate', $pagedata);
    }
    ...
    public function sendInvoice(Request $request)
    {
        ...
        if($request->invoice_id){
            ...
            // $validator = Validator::make($request->all(),[
            //     'send_invoice_to_emails' => 'bail|required|max:255',
            //     'send_invoice_subject' => 'bail|required|max:255',
            //     'send_invoice_message' => 'bail|required',
            // ]);
            // if ($validator->fails()) {
            //     return view('invoice.invoicecreate')->withErrors($validator)->with($pagedata);
            // }
            ...
            if (!empty($invoice_detail)) {
                $request->invoice_format = 'format001';
                ...
                $invpdf['inv'] = [
                    ...
                    'invoice_name' => $userinfo[1], 
                    'invoice_email' => $userinfo[2],
                    'invoice_phone' => $userinfo[1],
                    'invoice_terms' => $request->invoice_terms,
                    'invoice_format' => $request->invoice_format,
